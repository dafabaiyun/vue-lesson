    ...
    setup(props,ctx){
        ...
        return ()=>{
            ...
            return <ElDialog v-model={state.isShow} title={state.option.title}>
                {{
                    default:()=>
                        <ElInput type="textarea" v-model={state.option.content}
                            rows={10}
                        ></ElInput>,
...
export function $dialog(option){
    ...
    // 手动挂载组件  Vue2: new SubComponent.$mount()
    ...
    if(!vm){
        ...
        // 这里需要将el 渲染到页面中
        document.body.appendChild((render(vm,el),el))  //渲染成真实节点放到页面中

        
    }
    // 将组件渲染到这个el元素上
    let {showDialog}=vm.component.exposed
    showDialog(option)  //其他说明组件已经有了 只需要显示
