...
export function useCommand(data,focusData){
    ...
    const registry=(command)=>{
        ...
        state.commands[command.name]=(...args)=>{ //命令名字对应执行函数
            ...
            redo();
            if(!command.pushQueue){ return} //不需要放到队列中直接调过即可
        ...
        execute(){
            ...
            let after=(()=>{
                ...
                let maxZIndex=unfocused.reduce((prev,block)=>{
                    console.log(Math.max(prev,block.zIndex));
                    return Math.max(prev,block.zIndex);
                },-Infinity);

                focus.forEach(block=>block.zIndex=maxZIndex+1);
                console.log("block:"+JSON.stringify(data.value.blocks));
                return data.value.blocks
            })()
            return {
                undo:()=>{
                    // 如果当前blocks前后不一致，则不会更新
                    data.value={...data.value,blocks:before}
    ...
    registry({  //置底操作
        ...
        execute(){
            ...
            let after=(()=>{
                ...
                // 不能直接-1 因为index不能出现负值 负值就看不到组件了
                if(minZIndex<0){  //如果是负值则让没选中的向上，自己变成0
    ...
    registry({
        ...
        execute(){
            let state={
                ...
                after:focusData.value.unfocused //选中的删除 留下的是没选中的
        ...
        execute(newBlock,oldBlock){
            let state={
                ...
                after:(()=>{
                    ...
                    const index=data.value.blocks.indexOf(oldBlock)  //找老的需要通过老的查找
                    if(index>-1){
